		...
		<>
			...
			<div className='grid smLaptop:grid-cols-4 tablet:grid-cols-2 xsPhone:grid-cols-1 gap-10 w-full mt-10 '>
				{coreValues.map((value, idx) => {
					return (
						<div key={idx} className='w-full flex flex-col font-titilium gap-6'>
							<Image src={value.img} alt='' className='max-w-[90px]' />
							<h2 className='text-[#085C2C] smTablet:text-[min(4vh,4vw)] xsPhone:text-[min(6vw,6vh)]'>
								{value.name}
							</h2>
							<p className='text-neutral-600 smLaptop:text-[min(2vh,2vw)]  smTablet:text-[min(2.5vh,2.5vw)] xsPhone:text-[min(3.5vh,3.5vw)] font-light'>
								{value.desc}
							</p>
						</div>
					);
